"""
Isolated kernel benchmark, L2-FLUSH variant: default config vs. the
ORIGINAL, plain unpatched tuned config (./original-tuned-config/ --
the always-warm autotuner's own output, GROUP_SIZE_M=1 at every large-M
gate_up_proj anchor), timed under a direct L2 flush before every timed
launch, instead of the always-warm regime ../01-first-measurement/
originally used.

Why this variant exists: ../01-first-measurement/'s kernel-level benchmark
never left the operands warm in L2 between launches, and never showed the
original tuned config losing anywhere. Real serving (../02-nsys-shape-
profiling/) never gets a warm cache, and does show it losing, badly, on
gate_up_proj at large M. This script closes that gap: same 175-point grid
(5 shapes x 18 anchor + 17 held-out M values), same NUM_ITERS=2000, same
numerical sanity check as 01's script -- but with A/B/C flushed out of L2
before every single timed launch, the most direct simulation of "this
kernel never gets a warm cache" available. Only one B/Bs pair is needed
per point (unlike cache-cycling approaches that need many copies to force
eviction) -- the flush guarantees a cold cache regardless of what ran
immediately before.
"""

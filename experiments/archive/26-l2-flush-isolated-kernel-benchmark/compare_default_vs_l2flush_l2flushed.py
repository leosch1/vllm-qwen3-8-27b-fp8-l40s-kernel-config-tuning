"""
Isolated kernel benchmark, L2-FLUSH variant: default vs. this project's
L2-flush-tuned config (24), both timed under the SAME mechanism that
actually produced the winning config -- a direct L2 flush before every
timed launch (24's `benchmark_config_flush` design), not 17's model-specific
64-copy cycling that `compare_default_vs_l2flush_cycling.py` (this folder's
other script) used instead.

Why this variant exists: the cycling script validates the L2-flush-tuned
config under a DIFFERENT cache-pressure mechanism than the one that tuned
it -- a reasonable "does it generalize" check, but not the most direct
one. This script asks the more literal question: under the exact
methodology `24` used to pick this config, does it actually win? Using the
same mechanism for both tuning and validation also removes one variable
from any speedup-vs-cycling comparison -- if the two scripts' results
differ, that difference is attributable to the cache-pressure MECHANISM
itself (flush vs. cycling), not to a mismatch between what tuned the
config and what's now measuring it.
...
(a flushed cache carries no memory of what preceded it, by construction).

Same 175-point grid as 01/19/25's cycling script (5 shapes x 18 anchor + 17
held-out M values), same NUM_ITERS=2000, same numerical sanity check, same
documented /10 timing-bug scaling for parity with every other script in
this project.
"""
...
# Generous, documented fallback if this PyTorch build doesn't expose
# L2_cache_size -- same constant and reasoning as 24's l2_flush_tune.py.
...
L2FLUSH_DIR = "/tmp/l2-flush-tuned-configs"
NUM_ITERS = 2000

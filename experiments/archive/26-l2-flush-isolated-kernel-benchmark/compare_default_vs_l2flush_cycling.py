"""
Isolated kernel benchmark, cache-cycling variant: default vs. this
project's L2-flush-tuned config (24's rerun of the real autotuning script
with GROUP_SIZE_M's cache-locality blind spot fixed via direct L2 flushing
instead of 17's model-specific 64-copy cycling), both timed under the SAME
weight-tensor-cycling pressure (17's CyclingLaunchCounter design, 64
distinct B copies per point) -- the regime this fix actually targets,
exactly mirroring 19's validation of the cache-aware config.
...
def main():
    ...
    for N, K in SHAPES:
        ...
        print("| M | type | nearest anchor | default_cycled (us) | l2flush_cycled (us) | speedup |")
        print("|---:|---|---:|---:|---:|---:|")
        for M, ptype in points:
            ...
            l2flush_us = benchmark_config_cycling(

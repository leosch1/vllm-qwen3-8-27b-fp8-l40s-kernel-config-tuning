#!/usr/bin/env python3
"""
Build a singlefix c=64 SHAPE_MBAND_DATA series, directly comparable to
experiment 05's chart and to 20/21's groupfix/cache-aware panels.

`singlefix` (this experiment's config, see README.md) changes exactly one
value out of all 5 shapes' entire config: gate_up_proj's M=2048
GROUP_SIZE_M, 1 -> 16. Everywhere else it's byte-identical to the original
`tuned` config. GROUP_SIZE_M does NOT appear in the gridX/blockX formula at
all (gridX = ceil(M/BLOCK_SIZE_M)*ceil(N/BLOCK_SIZE_N), blockX =
num_warps*32) -- so singlefix's real launches have byte-identical
(gridX, blockX) signatures to the ORIGINAL tuned capture's launches, same
as groupfix. The existing tuned-configs/ lookup is reused unmodified for
classification (identical approach to 20's build_groupfix_mband.py).
...
CSV_PATH = "/tmp/singlefix_c64_launches.csv"
...
def main():
    ...
    with open("/tmp/singlefix_c64_durs.json", "w") as f:
        json.dump(durs, f)
    print("wrote /tmp/singlefix_c64_durs.json")

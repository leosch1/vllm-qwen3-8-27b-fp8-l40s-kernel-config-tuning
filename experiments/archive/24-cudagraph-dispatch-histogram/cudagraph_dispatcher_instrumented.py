...
#
# INSTRUMENTED COPY (experiment 23): the original `dispatch()` body is
# renamed to `_dispatch_uninstrumented` unchanged; the public `dispatch()`
# becomes a thin wrapper that additionally records (label, num_tokens,
# mode.name) into a Counter, periodically dumped to
# /tmp/dispatch_histogram.json -- same label-file/periodic-dump pattern as
# 10's fp8_utils.py _M_COUNTS instrumentation, for consistency.
#
# This call fires EXACTLY ONCE PER MODEL FORWARD STEP (both prefill and
# decode), for every step regardless of whether the resulting execution is
# graph-replayed (mode FULL/PIECEWISE) or eager (mode NONE) -- unlike the
# GEMM-kernel-wrapper counter in 10/03, which only ever sees eager calls
# (graph replay bypasses that Python call site entirely). Combined with
# each shape's real per-forward-pass layer count (64 for
# gate_up_proj/down_proj/out_proj, 48 for in_proj_qkvz, 16 for qkv_proj),
# this step-level (num_tokens, mode) count converts directly into exact
# per-shape kernel-launch counts by multiplication -- no gridX-decoding or
# graphId archaeology needed.
import json
import threading
import time
...
_STEP_COUNTS: Counter = Counter()  # (label, num_tokens, mode_name) -> count
_STEP_COUNTS_LOCK = threading.Lock()
_STEP_COUNTS_PATH = "/tmp/dispatch_histogram.json"
_LABEL_PATH = "/tmp/current_label.txt"
_CURRENT_LABEL = "unknown"
...
def _refresh_label():
    ...
    try:
        with open(_LABEL_PATH) as f:
            _CURRENT_LABEL = f.read().strip()
    except FileNotFoundError:
        pass


def _dump_step_counts():
    with _STEP_COUNTS_LOCK:
        snapshot = dict(_STEP_COUNTS)
    with open(_STEP_COUNTS_PATH, "w") as f:
        json.dump(
            {f"{label}|{n},{mode}": c for (label, n, mode), c in snapshot.items()}, f
        )


def _record_dispatch(num_tokens: int, mode_name: str):
    with _STEP_COUNTS_LOCK:
        _STEP_COUNTS[(_CURRENT_LABEL, num_tokens, mode_name)] += 1
...
class CudagraphDispatcher:
    ...
    def dispatch(
        ...
        """Instrumented wrapper -- see _dispatch_uninstrumented for the real
        (unmodified) logic. Records (label, num_tokens, mode.name) for every
        single call, i.e. every real forward step, whether it resolves to a
        graph replay (FULL/PIECEWISE) or eager (NONE) execution."""
        mode, desc = self._dispatch_uninstrumented(
            num_tokens,
            uniform_decode,
            has_lora,
            num_active_loras,
        ...
        _record_dispatch(num_tokens, mode.name)
        return mode, desc

    def _dispatch_uninstrumented(
        self,
        num_tokens: int,
        uniform_decode: bool = False,
        has_lora: bool = False,

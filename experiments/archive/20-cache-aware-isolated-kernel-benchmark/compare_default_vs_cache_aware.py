...
def main():
    ...
    for N, K in SHAPES:
        ...
        print("| M | type | nearest anchor | default (us) | cache_aware (us) | speedup | max output diff |")
        print("|---:|---|---:|---:|---:|---:|---:|")
        for M, ptype in points:
            ...
            cache_aware_us = benchmark_config(
            ...
            out_cache_aware = w8a8_block_matmul(A, B, As, Bs, [BLOCK_N, BLOCK_K], selected_config, OUT_DTYPE)
            max_diff = (out_default.float() - out_cache_aware.float()).abs().max().item()

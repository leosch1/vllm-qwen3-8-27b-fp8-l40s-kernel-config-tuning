"""
DIAGNOSTIC: same fine-grained interleaved design as
compare_tuned_vs_singlefix_m2048_interleaved.py, but with the ORDER
SWAPPED -- singlefix runs first in each round, tuned second (identical
weight tensor for both, same as before). The first version (tuned first,
singlefix second) gave +9.91% mean speedup, ~6x Part 9's between-repeat-
verified +1.74%. Hypothesis: whichever config runs SECOND in a round gets
a spurious cache-warming boost from the FIRST config having just pulled
the identical tensor into L2 microseconds earlier -- which would show up
as an inflated speedup for whichever config is placed second, regardless
of which one it is. If that's right, THIS run (singlefix first) should
show tuned getting the artificial boost instead -- i.e. the "speedup"
should shrink, flip sign, or otherwise clearly diverge from the first
run's +9.91%, confirming the effect is a placement artifact, not a real
GROUP_SIZE_M-driven difference.
"""
...
def benchmark_interleaved(A, As, copies, block_size, config_a, config_b, out_dtype, num_iters=2000):
    ...
    # warmup both, interleaved -- SWAPPED ORDER: config_b (singlefix) first, config_a (tuned) second
    for i in range(5):
...
def main():
    ...
    print()
    print(f"### gate_up_proj (N={N}, K={K}), M={M}, {N_REPEATS} repeats, ITERATION-LEVEL INTERLEAVED design, SWAPPED ORDER (singlefix first)")

"""
Isolated kernel benchmark, cache-cycling variant: default vs. the
ORIGINAL, unpatched tuned config (this repo's tuned-configs/ -- the
always-warm autotuner's own output, GROUP_SIZE_M=1 at gate_up_proj's
large-M anchors), both timed under 17's CyclingLaunchCounter design (64
distinct B copies per point, identical reset-to-index-0 sequence for both
configs) instead of the always-warm single-fixed-tensor methodology 01
used.

This is the most direct, rigorous test yet of 14's original mechanism
finding (tuned loses to default under cache pressure specifically because
of GROUP_SIZE_M) -- same 2000-iteration, numerically-verified methodology
as every other part of 19, just never previously pointed at the plain
unpatched tuned config.
...
def main():
    ...
    for N, K in SHAPES:
        ...
        print("| M | type | nearest anchor | default_cycled (us) | tuned_cycled (us) | speedup |")
        print("|---:|---|---:|---:|---:|---:|")

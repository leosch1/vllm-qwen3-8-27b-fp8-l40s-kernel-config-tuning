"""
Isolated kernel benchmark, cache-cycling variant: default vs. singlefix
(22's maximally-narrow config -- identical to the original tuned config
everywhere except gate_up_proj's single M=2048 entry, where GROUP_SIZE_M
changes from 1 to 16), timed under the SAME weight-tensor-cycling pressure
(17's CyclingLaunchCounter design, 64 distinct B copies per point) as 19's
other cycling comparisons -- lets singlefix be read against the same grid
as groupfix/cache-aware/tuned. Expectation to test: since singlefix only
differs from tuned at one specific M anchor, its speedup-vs-default curve
should track close to tuned's everywhere EXCEPT right around M=2048 (and
its nearest-anchor neighbors), where it should jump toward groupfix's
value there.
...
def main():
    ...
    for N, K in SHAPES:
        ...
        print("| M | type | nearest anchor | default_cycled (us) | singlefix_cycled (us) | speedup |")
        print("|---:|---|---:|---:|---:|---:|")
        for M, ptype in points:
            nearest = M if ptype == "anchor" else min(anchors, key=lambda x: abs(x - M))
            selected_config = singlefix_configs[nearest]
            ...
            speedup = (default_us - singlefix_us) / default_us * 100

            print(
                f"| {M} | {ptype} | {nearest} | {default_us:.2f} | {singlefix_us:.2f} "
                f"| {speedup:+.1f}% |"
            )

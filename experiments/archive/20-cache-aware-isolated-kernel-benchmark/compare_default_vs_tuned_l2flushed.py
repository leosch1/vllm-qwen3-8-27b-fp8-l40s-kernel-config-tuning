"""
Isolated kernel benchmark, L2-FLUSH variant: default vs. the ORIGINAL,
plain unpatched `tuned` config (this repo's `tuned-configs/`, the
always-warm autotuner's own output, `GROUP_SIZE_M=1` at every large-M
`gate_up_proj` anchor), timed under a direct L2 flush before every timed
launch (24's `benchmark_config_flush` mechanism) instead of 17's
model-specific 64-copy cycling that `compare_default_vs_tuned_cycling.py`
(this folder's sibling script, producing `chart-tuned-cycling-speedup.html`)
used.

Why this variant exists: this project has now measured the original
`tuned` config's isolated speedup under three different cache regimes --
always-warm (01, the original, most-favorable-to-`tuned` measurement),
cache-cycling (this folder's `compare_default_vs_tuned_cycling.py`), and
now L2-flush. Flush is the more general, model-independent mechanism this
project is proposing to replace 17's 64-copy cycling with (see 24's
README) -- checking whether `tuned`'s known regression looks the same,
better, or worse under it closes out the "does the choice of cache-
pressure mechanism itself change the headline finding" question for the
one config this whole investigation started with, not just for the
L2-flush-tuned config (25) already checked under both mechanisms.

Same 175-point grid as 01/19/25 (5 shapes x 18 anchor + 17 held-out M
values), same NUM_ITERS=2000, same numerical sanity check, same documented
/10 timing-bug scaling for parity with every other script in this project.
Only one `B`/`Bs` pair is needed per point (not 64 copies) -- the flush
before every timed launch guarantees a cold cache regardless of what ran
immediately before, so (unlike the cycling script) there's no need to
counterbalance run order between the two configs at each point either.
"""
...
# Generous, documented fallback if this PyTorch build's device-properties
# binding doesn't expose an L2-size field at all -- same constant/reasoning
# as 24's l2_flush_tune.py (attribute confirmed as `L2_cache_size`,
# underscored, not `L2CacheSize`, on this cluster's 2.13.0+cu130).
...
def main():
    ...
    for N, K in SHAPES:
        ...
        print("| M | type | nearest anchor | default_flushed (us) | tuned_flushed (us) | speedup |")
        print("|---:|---|---:|---:|---:|---:|")
        for M, ptype in points:
            ...
            tuned_us = benchmark_config_flush(

...
repo's committed tuned-configs/ (the output of ../04-l2-flush-retune/'s
retune), both timed under the SAME mechanism that actually produced the
winning config -- a direct L2 flush before every timed launch. Using the
same mechanism for tuning and validation means this script asks the most
literal possible question: under the exact condition the tuner optimized
for, does the config it picked actually win?
...
Same 175-point grid as ../01-first-measurement/'s script (5 shapes x 18
anchor + 17 held-out M values), same NUM_ITERS=2000, same numerical
sanity check, same documented /10 timing-bug scaling for parity.
"""
